...
const HomePage = ({ reload, setReload }) => {
  ...
  const handleSelectedGenres = (id, genre) => {
    ...
    if (localStorage.selectedGenresName) {
      let localStorageName = JSON.parse(
        localStorage.getItem("selectedGenresName")
      );
      localStorage.setItem(
        "selectedGenresName",
        JSON.stringify([...localStorageName, genre])
      );
    } else {
  ...
  const handleCancleGenres = (id, genre) => {
    ...
    let selectedGenresName = JSON.parse(
      localStorage.getItem("selectedGenresName")
    );
  ...
  return (
    <div
      className="min-h-screen bg-no-repeat w-full pt-20 bg-cover bg-black/70 bg-blend-overlay"
    ...
    >
      <div className="text text-center relative md:mt-32  lg:w-1/2 w-2/3 mx-auto">
        ...
        /> */}
        <div
          className={`text absolute cursor-pointer  bg-gray-950 mt-1 w-full mx-auto min-h-40 ${
            inputValue && closeSearchTray ? " " : "hidden"
          }`}
        >
          {searchM.map((value) => (
            <>
              <div
                onClick={() => handleSearchInput(value)}
                className="text hover:bg-slate-50 border px-5 py-1"
              >
                {value.title}
              </div>
            </>
      ...
      <div className="text-white w-full lg:w-4/5 mx-auto mt-10 lg:px-40  px-2 md:px-10">
        ...
        <div className="text-center mt-3">
          <button
            onClick={handleRecommendationRoute}
            className={`text-center justify-center bg-blue-600 px-5 w-1/2 md:w-1/3 mx-auto py-2 items-center rounded  ${
              selectedGenres?.length > 0 ? "flex" : " hidden "
            }`}
          >
            Explore
            <MdArrowForwardIos
              className={`ml-3 font-bold text-lg animate-pulse `}
            />
          </button>
        </div>
          ...
          {Object.keys(genres).map((id) => (
            <div className=" flex justify-center  mx-auto">
              ...
              {selectedGenres?.includes(id) ? (
                <button
                  onClick={() => handleCancleGenres(id, genres[id])}
                  className="md:mx-5 mx-1 font-semibold text-sm text-black bg-white md:px-5 px-3 py-2 my-3 flex items-center gap-1 rounded-lg"
                  key={id}
                >
                ...
                <button
                  onClick={() => handleSelectedGenres(id, genres[id])}
                  className="md:mx-5 mx-1 font-semibold text-sm bg-[rgb(255,0,47)] md:px-5 px-3 py-2 my-3 flex items-center gap-1 rounded-lg"
                  key={id}
                >
                  <p className="text">{genres[id]}</p>
